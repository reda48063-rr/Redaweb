...
const LSK_VISITS='luxS_visits';
const LSK_BOOKINGS='luxS_bookings';
const LSK_DATE='luxS_last';
...
export function bootCounters(){
  ...
  document.querySelectorAll('[data-kpi=visits] b').forEach(el=>el.textContent=fmt(v));
  const arr = JSON.parse(localStorage.getItem(LSK_BOOKINGS)||'[]');
  document.querySelectorAll('[data-kpi=bookings] b').forEach(el=>el.textContent=fmt(arr.length));
...
export function loadAdmin(){
  ...
  tbody.innerHTML = arr.map(b => `<tr><td>${b.id}</td><td>${b.name}</td><td>${b.phone}</td><td>${b.room}</td><td>${b.checkin}</td><td>${b.checkout}</td></tr>`).join('');
  ...
  if(ctx && window.Chart){
    ...
    new Chart(ctx, { type:'bar', data:{ labels:days, datasets:[{ label:'الحجوزات/أسبوع', data:counts }] }, options:{ plugins:{legend:{display:false}}, scales:{ y:{ beginAtZero:true } } } });
  }
}
export function initSlider(){
  ...
  let idx = 0;
  function paint(){ el.src = 'assets/img/' + imgs[idx]; dots.forEach((d,i)=> d.classList.toggle('active', i===idx)); }
  paint();
